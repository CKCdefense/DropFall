 ...
 * 대역폭 쪽으로 다시 낮추는 실험을 시작한다 — 20(이미 검증된 안전값)으로 한 번에 가는
 * 대신 60에서 50으로 먼저 낮춰서, 어느 지점부터 다시 끊기는지 좁혀본다. **50도 아직
 * 다인 실접속으로 검증 전이다** — 끊기면 더 낮추고(40→20 순으로), 괜찮으면 계속
 * 낮춰서 어디까지 버티는지 찾는다. 단일 상수라 되돌리기 쉽다.
 */
