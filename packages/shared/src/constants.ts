 ...
 * 조작 자체가 느리게 느껴진다**는 문제가 나왔다 — 이 클라이언트는 자기 캐릭터도
 * 클라이언트 예측(prediction) 없이 서버 스냅샷을 그대로 보간해서 그리므로
 * (`EntityRenderer.syncPlayers`가 로컬/원격을 구분하지 않음), PATCH_RATE가 곧
 * "내 입력이 화면에 반영되기까지 걸리는 지연"이기도 하다 — 원래(60Hz) 33ms였던 게
 * 20Hz에서 100ms가 되어 체감상 확실히 느려졌다. 자기 캐릭터 예측은 아직 없다
 * (로드맵엔 있었지만 미구현, docs/backend/47 다음 작업).
 *
 * 지금은 서버의 WebSocket 압축(permessage-deflate, `packages/server/src/index.ts`)을
 * 같이 켜서 확보한 대역폭 여유를 반응성 쪽에 다시 썼다 — 40으로 한 차례 올렸다가,
 * TICK_RATE와 같은 60까지 실험적으로 다시 올렸다. **압축률을 실측한 게 아니다** —
 * Colyseus 상태 전송은 이미 컴팩트한 바이너리 diff라 반복 텍스트가 많은 일반 JSON보다
 * 압축이 덜 먹힐 수 있어서, 이 값이 "60Hz 무압축 2인 렉"을 다시 밟을지는 이론만으론
 * 장담 못 한다 — 실제 2인 이상 접속으로 체감 확인 후 판단할 것. 다시 끊기면 이전에
 * 이미 검증된 40(또는 그 아래)으로 낮춘다 — 단일 상수라 되돌리기 쉽다.
 *
 * 자기 캐릭터 예측을 구현하면 이 값은 순수하게 "원격 개체 부드러움 vs 대역폭"
 * 트레이드오프로만 남게 되어 인원수 걱정 없이 낮출 수 있다(다음 과제).
 */
export const PATCH_RATE = 60;
