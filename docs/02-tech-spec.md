...
| **상태 전송** | **50Hz**(`PATCH_RATE`) — **다인 대역폭 재검증 중, 진행형 실험.** Colyseus `Room.patchRate`는 시뮬레이션 틱(60Hz)과 **별개**이고 기본값이 20Hz다 — 명시하지 않으면 서버를 60Hz로 돌려도 클라이언트는 20Hz로만 본다([frontend/05](frontend/05-work-report-patch-rate.md)). 패치는 클라이언트 소켓마다 독립적으로 나가 대역폭이 인원수에 비례하는 탓에, 한때 60Hz에서 2인 이상 실접속이 심하게 끊겨 20Hz로 낮춘 적이 있다([backend/47](backend/47-work-report-patch-rate-bandwidth.md)). 그런데 20Hz에선 이번엔 자기 캐릭터 조작 반응이 느려지는 문제가 나와(예측이 아직 없던 시절, 서버 패치를 그대로 그렸다) 다시 60까지 올렸는데, 다인 대역폭 재검증 없이 방치됐다. 자기 캐릭터 클라이언트 예측(§4.2, [backend/55](backend/55-work-report-player-client-prediction.md))이 그 뒤에 들어와서 반응성 문제가 PATCH_RATE와 무관해졌으므로([backend/58](backend/58-work-report-session-handoff-verification.md)), 다시 대역폭 쪽으로 낮추는 실험을 시작했다 — 20(이미 검증된 안전값)으로 바로 가지 않고 60→**50**부터 단계적으로 낮춰 어디서 다시 끊기는지 좁혀본다. **50도 아직 다인 실접속 검증 전이다** — 끊기면 더 낮추고, 괜찮으면 계속 낮춘다. 단일 상수라 되돌리기 쉽다(`packages/shared/src/constants.ts` 주석 참고) |
| 클라 렌더 | 60fps |
| 권위 | **서버 100%** |
