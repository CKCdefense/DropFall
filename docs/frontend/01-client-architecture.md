...
아트가 없어서 지금은 전부 도형 플레이스홀더다 — 플레이어 / 몬스터 / 투사체.
몬스터는 타입별 색·크기를 `MONSTER_STYLE` 표 하나로 관리하므로, 아틀라스가 준비되면
그 표와 `create*` 메서드만 스프라이트 키로 바꾸면 된다.
...
- 몬스터·투사체·코어HP·웨이브는 **도형 플레이스홀더**로 그린다.
  `EntityRenderer`의 `MONSTER_STYLE` 표와 `create*` 메서드만 스프라이트로 교체하면 된다
